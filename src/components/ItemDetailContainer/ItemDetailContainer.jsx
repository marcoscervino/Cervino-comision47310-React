...
export function ItemDetailContanier () {
    const params = useParams();
  const [producto, setProducto] = useState({});
  const [cargando, setCargando] = useState(true);

  useEffect(() => {
    simularCargaDeProducto().then(productos => {
      const productoEncontrado = productos.find(item => item.id === parseInt(params.id));
      setProducto(productoEncontrado);
      setCargando(false);
    });
  }, [params.id]);

  const simularCargaDeProducto = () => {
